    ...
    <div class="bg-white rounded-lg shadow overflow-hidden">
      ...
      <div v-if="lastPage > 1" class="bg-white px-4 py-3 flex items-center justify-between border-t border-gray-200 sm:px-6">
        <div class="flex-1 flex justify-between sm:hidden">
          <button @click="page > 1 && (page--, fetchMessages())" :disabled="page <= 1" class="relative inline-flex items-center px-4 py-2 border border-gray-300 text-sm font-medium rounded-md text-gray-700 bg-white hover:bg-gray-50 disabled:opacity-50">Previous</button>
          <button @click="page < lastPage && (page++, fetchMessages())" :disabled="page >= lastPage" class="ml-3 relative inline-flex items-center px-4 py-2 border border-gray-300 text-sm font-medium rounded-md text-gray-700 bg-white hover:bg-gray-50 disabled:opacity-50">Next</button>
        </div>
      ...
      <div class="bg-white rounded-lg shadow-xl max-w-lg w-full mx-4">
        ...
        <form @submit.prevent="sendMessage">
          ...
          <div class="px-6 py-4 border-t border-gray-200 flex justify-end gap-3">
            ...
            <button type="submit" :disabled="saving" class="px-4 py-2 bg-indigo-600 text-white rounded-md hover:bg-indigo-700 text-sm font-medium disabled:opacity-50">{{ saving ? 'Sending...' : 'Send Message' }}</button>
          </div>
        </form>
      </div>
    </div>
...
const fetchMessages = async () => {
  try {
    const r = await api.get('/teacher/messages', { params: { page: page.value, type: activeTab.value } });
...
const sendMessage = async () => {
  saving.value = true;
  try {
    await api.post('/teacher/messages', form.value);
...
const markRead = async (id) => {
  try {
    await api.post(`/teacher/messages/${id}/read`);
